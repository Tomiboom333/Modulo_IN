...
int main(void)
{
  ...
  while (1)
  {
    ...
    HAL_SPI_Receive(&hspi1, (uint8_t*)RxBuffer, 4, 100); 
    ...
    for (int i = 4; i < 8; i++)
    {
     ...
     for (int i = 0; i < 3; i++) {//asegurarse que lo demas es 0
        TxBuffer[i] = 0x00;
    }
    for (int i = 0; i < 8; i++) {
    ...
    if(HAL_SPI_Transmit(&hspi1, (uint8_t*)TxBuffer, 3, 100) == HAL_OK) HAL_GPIO_WritePin(GPIOB, GPIO_PIN_9, 0);
